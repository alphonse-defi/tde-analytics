...
function detectConcentration(holders, comparison) {
  ...
  // Gini coefficient (inline calc for insight engine independence)
  ...
  if (balances.length >= 2) {
    ...
    if (sum > 0) {
      let numerator = 0;
      for (let i = 0; i < n; i++) numerator += (2 * (i + 1) - n - 1) * balances[i];
      const gini = Math.max(0, Math.min(1, numerator / (n * sum)));

      if (gini > 0.85) {
        insights.push({
          category: CATEGORY.CONCENTRATION, severity: SEVERITY.CRITICAL,
          title: 'Extreme holder concentration',
          message: `Gini coefficient is ${gini.toFixed(3)} — token supply is heavily concentrated in few wallets.`,
        ...
        insights.push({
          ...
          message: `Gini coefficient is ${gini.toFixed(3)} — supply distribution is significantly unequal.`,
          metric: `Gini ${gini.toFixed(3)}`
        });
      }
    }
