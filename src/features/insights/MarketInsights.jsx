...
const MarketInsights = memo(({ onSelectToken, trackedTokens }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // CoinGecko: fetch 100 instead of 250 to reduce rate limit pressure
      const market = await fetchFromCoinGecko(
        ...
        console.warn('CoinGecko market fetch failed (may be rate limited):', err.message);
        return [];
      });

      // Nansen: try ethereum only first (1 chain = faster, less likely to 504)
      // Only attempt if we got market data (avoid hammering APIs when one is down)
      let sm = [];
      try {
        sm = await getSmartMoneyHoldings(['ethereum']);
      } catch (nansenErr) {
        console.warn('Nansen SM holdings failed (non-blocking):', nansenErr.message);
        // SM signals just won't appear — page still works
      }
